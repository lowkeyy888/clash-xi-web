...
const COIN_BUNDLE_FALLBACK = [
 { id: "fc_001", name: "Starter Bundle", fcAmount: 1000, amountCents: 99, currency: "usd", bonusLabel: "Kickoff entry", bestValue: false },
 { id: "fc_002", name: "Supporter Bundle", fcAmount: 2500, amountCents: 299, currency: "usd", bonusLabel: "+250 bonus", bestValue: false },
 { id: "fc_003", name: "Club Bundle", fcAmount: 5500, amountCents: 499, currency: "usd", bonusLabel: "+500 bonus", bestValue: false },
 { id: "fc_004", name: "Manager Bundle", fcAmount: 12000, amountCents: 999, currency: "usd", bonusLabel: "+1,500 bonus", bestValue: false },
 { id: "fc_005", name: "Owner Bundle", fcAmount: 30000, amountCents: 2499, currency: "usd", bonusLabel: "+5,000 bonus", bestValue: false },
 { id: "fc_006", name: "Chairman Bundle", fcAmount: 100000, amountCents: 7999, currency: "usd", bonusLabel: "+20,000 bonus • Best Value", bestValue: true }
];
...
function getCoinBundleById(bundleId) {
 const list = (state.market.coinBundles?.length ? state.market.coinBundles : COIN_BUNDLE_FALLBACK).map(normalizeCoinBundle);
...
function renderHomeLeagueStanding(runtime) {
 ...
 <div>${row.pos}</div>
 <div>${row.name}${row.bot ? ` <span class="botTag">BOT</span>` : ""}</div>
 <div>${row.pts}</div>
 </div>
 `).join("")}
 `;
...
function setMarketTab(tab) {
 ...
 if (nextTab === "coins") renderCoinExchangeShell();
}
...
function initCoinExchangeShell() {
 ...
 $$(".coinExchangeRow", grid).forEach((row) => {
 const activate = () => {
 state.market.selectedCoinBundle = row.dataset.coinBundle || null;
...
function renderCoinExchangeShell() {
 ...
 const bundles = (state.market.coinBundles?.length ? state.market.coinBundles : COIN_BUNDLE_FALLBACK).map(normalizeCoinBundle);
 if (state.market.selectedCoinBundle && !bundles.some((b) => b.id === state.market.selectedCoinBundle)) {
 ...
 return `
 <div class="coinBundleCard coinExchangeRow ${selected ? "is-selected" : ""}" data-coin-bundle="${bundle.id}" style="align-content:stretch;justify-items:stretch;gap:14px;${selected ? "border-color: var(--border-bright); box-shadow: 0 0 0 1px rgba(232,184,75,.20), 0 0 24px rgba(232,184,75,.12);" : ""}">
 <div style="display:flex;justify-content:space-between;gap:14px;align-items:flex-start;flex-wrap:wrap">
 <div style="display:grid;gap:6px;min-width:0">
 <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap">
 <div class="marketPlaceholderTitle" style="font-size:24px">${bundle.name}</div>
 ${bundle.bestValue ? '<div class="tag">Best Value</div>' : ""}
 </div>
 <div class="muted tiny">${bundle.fcAmount.toLocaleString("en-GB")} FC${bundle.bonusLabel ? ` • ${bundle.bonusLabel}` : ""}</div>
 </div>
 <div style="display:grid;justify-items:end;gap:10px;min-width:120px">
 <div class="coinBundlePrice">${usdFmtFromCents(bundle.amountCents)}</div>
 <button class="smallBtn ${selected ? "primary" : "ghost"} coinBundleSelectBtn" data-coin-select="${bundle.id}">${selected ? "Selected" : "Select"}</button>
 </div>
 </div>
 </div>`;
 }).join("");
 ...
 notesWrap.innerHTML = `
 <div class="marketPanel" style="max-width:600px;margin:0 auto;display:grid;gap:12px;">
 <div class="row">
 <div>
 <div class="sectionTitle">Coin Exchange</div>
 <div class="muted tiny">Stripe-hosted checkout for real-money Forge Coin purchases.</div>
 </div>
 ...
 <div class="listBox">
 <div class="listRow">
 <div class="muted tiny">Selected Bundle</div>
 <div style="font-weight:900">${selectedBundle?.name || "None"}</div>
 </div>
 <div class="muted tiny" style="margin-top:8px">${selectedBundle ? `${selectedBundle.fcAmount.toLocaleString("en-GB")} FC • ${usdFmtFromCents(selectedBundle.amountCents)}` : "Choose one bundle to unlock checkout."}</div>
 <button class="btn primary" id="btn-confirm-coin-purchase" ${state.market.selectedCoinBundle && !state.market.coinCheckoutBusy ? "" : "disabled"} style="width:100%;margin-top:12px">${state.market.coinCheckoutBusy ? "Redirecting…" : "Confirm Purchase"}</button>
 </div>

 <div class="muted tiny">Monthly spend: ${usdFmtFromCents(state.market.monthlySpendCents)} / ${usdFmtFromCents(state.market.spendCapCents)} cap</div>
 ${state.market.coinExchangeError ? `<div class="muted tiny" style="color:var(--alert-red)">${state.market.coinExchangeError}</div>` : ""}
 </div>`;
...
async function handleCheckoutReturn() {
 ...
 setStatus("Stripe checkout completed. Refreshing Coin Exchange and wallet…");
